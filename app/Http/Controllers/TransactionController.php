...
class TransactionController extends Controller
{
    public function index()
    {
        $transactions = TransactionUnit::with(['transaction', 'unit'])
            ...
            ->map(function ($transaction) {
                ...
                $transaction->fine = round($remainingDays) * 5000;
                return $transaction;
            });
    ...
    public function returnForm($transaction_code, $unit_id)
    {
        ...
        $transactionUnit->fine = abs(round($remainingDays) * 5000); // Denda per hari

        return view('admin.transactions.returns', compact('transactionUnit'));
    }
    ...
    public function show($transaction_code)
    {
        ...
        foreach ($transaction->units as $unit) {
            ...
            $dateReturned = $unit->date_returned ? \Carbon\Carbon::parse($unit->date_returned) : now();
            $daysRented = abs(round($dateBorrowed->diffInDays($dateReturned)));
            $itemPrice = $daysRented * $unit->unit->harga; // Assuming harga is in the Unit model

            $itemsWithPrices[] = [
                'name' => $unit->unit->name, // Get unit name
